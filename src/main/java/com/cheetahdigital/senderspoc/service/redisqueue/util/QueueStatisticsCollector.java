...
public class QueueStatisticsCollector {
  ...
  /**
   * Retrieves the current failure count we have in memory for this redisques instance (there might
   * be more than one in use).
   *
   * <p>Note: Because each queue is associated resp. registered with a certain redisques instance,
   ...
   * all queues requested (independent of the redisques instance for which the queues are
  ...
  public void getQueueStatistics(Message<JsonObject> event, final List<String> queues) {
    if (queues == null || queues.isEmpty()) {
      ...
      event.reply(new JsonObject().put(STATUS, OK).put(RedisQueueAPI.QUEUES, new JsonArray()));
      return;
    }
        ...
        queueListLength -> {
          ...
          redisAPI.hvals(
              ...
              statisticsSet -> {
                ...
                event.reply(
                    new JsonObject()
                        .put(RedisQueueAPI.STATUS, RedisQueueAPI.OK)
                        .put(RedisQueueAPI.QUEUES, result));
              });
        });
  }
   ...
   *
   * <p>Note: This method only treats queue speed values from the current redisques instance. There
