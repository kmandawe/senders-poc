...
public class RedisQueuesAPI {
  ...
  private static Logger log = LoggerFactory.getLogger(RedisQueuesAPI.class);
  ...
  public enum QueueOperation {
    ...
    public static QueueOperation fromString(String op) {
      for (QueueOperation queueOperation : values()) {
        ...
        } else if (queueOperation.hasLegacyName()
            && queueOperation.getLegacyName().equalsIgnoreCase(op)) {
          log.warn(
              "Legacy queue operation used. This may be removed in future releases. Use '"
                  + queueOperation.name()
                  + "' instead of '"
                  + queueOperation.getLegacyName()
                  + "'");
          return queueOperation;
        }
      }
  ...
  public static JsonObject buildSetConfigurationOperation(JsonObject configuration) {
    return buildOperation(QueueOperation.setConfiguration, configuration);
  }
  ...
  public static JsonObject buildEnqueueOperation(String queueName, String message) {
    JsonObject operation =
        buildOperation(QueueOperation.enqueue, new JsonObject().put(QUEUENAME, queueName));
    operation.put(MESSAGE, message);
    return operation;
  }

  public static JsonObject buildEnqueueOperation(String queueName, JsonObject payload) {
    JsonObject operation =
        buildOperation(QueueOperation.enqueue, new JsonObject().put(QUEUENAME, queueName));
    operation.put(MESSAGE, payload);
    return operation;
  }

  public static JsonObject buildLockedEnqueueOperation(
      String queueName, String message, String lockRequestedBy) {
    JsonObject operation =
        buildOperation(
            QueueOperation.lockedEnqueue,
            new JsonObject().put(QUEUENAME, queueName).put(REQUESTED_BY, lockRequestedBy));
    operation.put(MESSAGE, message);
    return operation;
  }
  ...
  public static JsonObject buildGetQueueItemsOperation(String queueName, String limit) {
    return buildOperation(
        QueueOperation.getQueueItems, new JsonObject().put(QUEUENAME, queueName).put(LIMIT, limit));
  }

  public static JsonObject buildAddQueueItemOperation(String queueName, String buffer) {
    return buildOperation(
        QueueOperation.addQueueItem,
        new JsonObject().put(QUEUENAME, queueName).put(BUFFER, buffer));
  }

  public static JsonObject buildGetQueueItemOperation(String queueName, int index) {
    return buildOperation(
        QueueOperation.getQueueItem, new JsonObject().put(QUEUENAME, queueName).put(INDEX, index));
  }

  public static JsonObject buildReplaceQueueItemOperation(
      String queueName, int index, String buffer) {
    return buildOperation(
        QueueOperation.replaceQueueItem,
        new JsonObject().put(QUEUENAME, queueName).put(INDEX, index).put(BUFFER, buffer));
  }

  public static JsonObject buildDeleteQueueItemOperation(String queueName, int index) {
    return buildOperation(
        QueueOperation.deleteQueueItem,
        new JsonObject().put(QUEUENAME, queueName).put(INDEX, index));
  }
  ...
  public static JsonObject buildDeleteAllQueueItemsOperation(String queueName, boolean unlock) {
    return buildOperation(
        QueueOperation.deleteAllQueueItems,
        new JsonObject().put(QUEUENAME, queueName).put(UNLOCK, unlock));
  }

  public static JsonObject buildBulkDeleteQueuesOperation(JsonArray queuesToDelete) {
    return buildOperation(
        QueueOperation.bulkDeleteQueues, new JsonObject().put(QUEUES, queuesToDelete));
  }
  ...
  /** @param filterPattern Filter pattern. Method handles {@code null} gracefully. */
  ...
  public static JsonObject buildGetQueuesCountOperation(String filterPattern) {
    if (filterPattern != null) {
      return buildOperation(
          QueueOperation.getQueuesCount, new JsonObject().put(FILTER, filterPattern));
    } else {
      return buildOperation(QueueOperation.getQueuesCount);
    }
  }
  ...
  public static JsonObject buildGetQueueItemsCountOperation(String queueName) {
    return buildOperation(
        QueueOperation.getQueueItemsCount, new JsonObject().put(QUEUENAME, queueName));
  }
  ...
  public static JsonObject buildBulkDeleteLocksOperation(JsonArray locksToDelete) {
    return buildOperation(
        QueueOperation.bulkDeleteLocks, new JsonObject().put(LOCKS, locksToDelete));
  }

  public static JsonObject buildPutLockOperation(String queueName, String user) {
    return buildOperation(
        QueueOperation.putLock, new JsonObject().put(QUEUENAME, queueName).put(REQUESTED_BY, user));
  }

  public static JsonObject buildBulkPutLocksOperation(JsonArray locksToPut, String user) {
    return buildOperation(
        QueueOperation.bulkPutLocks,
        new JsonObject().put(LOCKS, locksToPut).put(REQUESTED_BY, user));
  }
  ...
  public static JsonObject buildGetAllLocksOperation(String filterPattern) {
    if (filterPattern != null) {
      return buildOperation(
          QueueOperation.getAllLocks, new JsonObject().put(FILTER, filterPattern));
    } else {
  ...
  public static JsonObject buildGetQueuesStatisticsOperation(String filterPattern) {
    if (filterPattern != null) {
      return buildOperation(
          QueueOperation.getQueuesStatistics, new JsonObject().put(FILTER, filterPattern));
    }
   ...
   *
   * @param filterPattern The queues filter for which we would like to get the total speed Filter
   *     pattern. Method handles {@code null} gracefully.
   */
  public static JsonObject buildGetQueuesSpeedOperation(String filterPattern) {
    if (filterPattern != null) {
      return buildOperation(
          QueueOperation.getQueuesSpeed, new JsonObject().put(FILTER, filterPattern));
    }
