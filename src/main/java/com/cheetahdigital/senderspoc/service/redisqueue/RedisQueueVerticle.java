package com.cheetahdigital.senderspoc.service.redisqueue;

import com.cheetahdigital.senderspoc.service.redisqueue.handler.*;
import com.cheetahdigital.senderspoc.service.redisqueue.lua.LuaScriptManager;
import com.cheetahdigital.senderspoc.service.redisqueue.util.*;
...
@Slf4j
public class RedisQueueVerticle extends AbstractVerticle {
  ...
  public void start() {
    ...
    RedisQueueConfiguration modConfig = RedisQueueConfiguration.fromJsonObject(config());
    ...
    timer = new RedisQueueTimer(vertx);
    ...
    RedisQueueHttpRequestHandler.init(vertx, modConfig);
    ...
    eb.consumer(
        ...
        (Handler<Message<JsonObject>>)
            event -> {
              ...
              RedisQueueAPI.QueueOperation queueOperation =
                  RedisQueueAPI.QueueOperation.fromString(operation);
                    ...
                    redisAPI.get(
                        ...
                        getConsumerEvent -> {
                          ...
                          if (uid.equals(consumer)) {
                            log.debug(
                                "RedisQues Periodic consumer refresh for active queue " + queue);
    ...
    redisAPI.rpush(
        ...
        event2 -> {
          ...
          if (event2.succeeded()) {
            ...
            String message =
                "RedisQues QUEUE_ERROR: Error while enqueueing message into queue " + queueName;
  ...
  private void lockedEnqueue(Message<JsonObject> event) {
    ...
    if (lockInfo != null) {
      ...
      log.warn(
          "RedisQues lockedEnqueue failed because property '" + REQUESTED_BY + "' was missing");
  ...
  private void getConfiguration(Message<JsonObject> event) {
    ...
    result.put(RedisQueueConfiguration.PROP_ADDRESS, address);
    result.put(RedisQueueConfiguration.PROP_REDIS_PREFIX, redisPrefix);
    result.put(RedisQueueConfiguration.PROP_PROCESSOR_ADDRESS, processorAddress);
    result.put(RedisQueueConfiguration.PROP_REFRESH_PERIOD, refreshPeriod);
    result.put(RedisQueueConfiguration.PROP_REDIS_HOST, redisHost);
    result.put(RedisQueueConfiguration.PROP_REDIS_PORT, redisPort);
    result.put(RedisQueueConfiguration.PROP_REDIS_AUTH, redisAuth);
    result.put(RedisQueueConfiguration.PROP_REDIS_ENCODING, redisEncoding);
    result.put(RedisQueueConfiguration.PROP_CHECK_INTERVAL, checkInterval);
    result.put(RedisQueueConfiguration.PROP_PROCESSOR_TIMEOUT, processorTimeout);
    result.put(RedisQueueConfiguration.PROP_PROCESSOR_DELAY_MAX, processorDelayMax);
    result.put(
        RedisQueueConfiguration.PROP_HTTP_REQUEST_HANDLER_ENABLED, httpRequestHandlerEnabled);
    result.put(RedisQueueConfiguration.PROP_HTTP_REQUEST_HANDLER_PREFIX, httpRequestHandlerPrefix);
    result.put(RedisQueueConfiguration.PROP_HTTP_REQUEST_HANDLER_PORT, httpRequestHandlerPort);
    result.put(
        RedisQueueConfiguration.PROP_HTTP_REQUEST_HANDLER_USER_HEADER,
  ...
  private void registerQueueCheck(RedisQueueConfiguration modConfig) {
    vertx.setPeriodic(
    ...
    redisAPI.hexists(
        ...
        event -> {
          if (event.failed()) {
            ...
            // Previous implementation assumed this. See
            // "https://github.com/hiddenalpha/vertx-redisques/blob/v2.5.1/src/main/java/org/swisspush/redisques/RedisQues.java#L856".
                              ...
                              if (success) {
                                ...
                                redisAPI.lpop(
                                    ...
                                    jsonAnswer -> {
                                      ...
                                      log.debug(
                                          "RedisQues Message removed, queue "
                                              + queueName
                                              + " is ready again");
                                      myQueues.put(queueName, QueueState.READY);
                                ...
                                if (log.isDebugEnabled()) {
                                  ...
                                  // reschedule
                                  log.debug(
                                      "RedisQues will re-send the message to queue '"
                                          + queueName
                                          + "' in "
                                          + retryInterval
                                          + " seconds");
  ...
  private void rescheduleSendMessageAfterFailure(final String queueName, int retryInSeconds) {
    ...
    vertx.setTimer(
        ...
        timerId -> {
          if (log.isDebugEnabled()) {
            log.debug(
                "RedisQues re-notify the consumer of queue '"
                    + queueName
                    + "' at "
                  ...
                  (Handler<AsyncResult<Message<JsonObject>>>)
                      reply -> {
                        ...
                        if (reply.succeeded()) {
                          ...
                          log.info(
                              "RedisQues QUEUE_ERROR: Consumer failed "
                                  + uid
                                  + " queue: "
                                  + queue
                                  + " ("
    ...
    redisAPI.get(
        ...
        event -> {
          ...
          if (consumer == null) {
            ...
            log.debug(
                "RedisQues Notifying consumer " + consumer + " to consume queue " + queueName);
  ...
  private void refreshRegistration(String queueName, Handler<AsyncResult<Response>> handler) {
    if (log.isDebugEnabled()) {
      log.debug(
          "RedisQues Refreshing registration of queue "
              + queueName
              + ", expire in "
              + consumerLockTime
              + " s");
            ...
            redisAPI.exists(
                ...
                event -> {
                  if (event.failed() || event.result() == null) {
                    log.error(
                        "RedisQues is unable to check existence of queue " + queueName,
                        event.cause());
                    return;
                  }
