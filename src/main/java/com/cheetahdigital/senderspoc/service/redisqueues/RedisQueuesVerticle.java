package com.cheetahdigital.senderspoc.service.redisqueues;

import com.cheetahdigital.senderspoc.service.redisqueues.handler.*;
import com.cheetahdigital.senderspoc.service.redisqueues.lua.LuaScriptManager;
import com.cheetahdigital.senderspoc.service.redisqueues.util.*;
...
@Slf4j
public class RedisQueuesVerticle extends AbstractVerticle {
  ...
  // Address of this redisQueues. Also used as prefix for consumer broadcast
  // address.
  private String address = "redis-queues";

  private String configurationUpdatedAddress = "redis-queues-configuration-updated";
  ...
  private String processorAddress = "redis-queues-processor";
  ...
  private RedisQueuesTimer timer;
  ...
  private static final Set<String> ALLOWED_CONFIGURATION_VALUES =
      Stream.of("processorDelayMax").collect(Collectors.toSet());

  private LuaScriptManager luaScriptManager;

  private void redisSetWithOptions(
      String key, String value, boolean nx, int ex, Handler<AsyncResult<Response>> handler) {
    ...
    redisAPI
        .send(Command.SET, RedisUtils.toPayload(key, value, options).toArray(new String[0]))
        .onComplete(handler);
  }
  ...
  private void handleRegistrationRequest(Message<String> msg) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "RedisQueues Got registration request for queue {} from consumer: {}", queueName, uid);
    }
    // Try to register for this queue
    redisSetWithOptions(
        consumersPrefix + queueName,
        uid,
        true,
        consumerLockTime,
        event -> {
          if (event.succeeded()) {
            ...
            if (log.isTraceEnabled()) {
              log.trace("RedisQueues setxn result: " + value + " for queue: " + queueName);
            }
            if ("OK".equals(value)) {
              // I am now the registered consumer for this queue.
              if (log.isDebugEnabled()) {
                log.debug("RedisQueues Now registered for queue " + queueName);
              }
              myQueues.put(queueName, QueueState.READY);
              consume(queueName);
            } else {
              log.debug("RedisQueues Missed registration for queue " + queueName);
  ...
  public void start() {
    ...
    redisPrefix = modConfig.getRedisPrefix(); // default: "redis-queues:"
    ...
    consumerLockTime =
        2
            * refreshPeriod; // lock is kept twice as long as its refresh interval -> never expires
                             // as long as the consumer ('we') are alive
    ...
    timer = new RedisQueuesTimer(vertx);
    ...
    this.queueStatisticsCollector =
        new QueueStatisticsCollector(
            redisAPI, luaScriptManager, queuesPrefix, vertx, modConfig.getQueueSpeedIntervalSec());

    RedisQueuesHttpRequestHandler.init(vertx, modConfig);

    eb.consumer(
        configurationUpdatedAddress,
    ...
    eb.consumer(
        ...
        (Handler<Message<JsonObject>>)
            event -> {
              ...
              if (null == body) {
                log.warn(
                    "Got msg with empty body from event bus. We'll run directly in a NullPointerException now. address={}  replyAddress={} ",
                    event.address(),
                    event.replyAddress());
                // IMO we should 'fail()' here. But we don't, to keep backward compatibility.
              }
              ...
              if (log.isTraceEnabled()) {
                log.trace("RedisQueues got operation:" + operation);
              }

              RedisQueuesAPI.QueueOperation queueOperation =
                  RedisQueuesAPI.QueueOperation.fromString(operation);
              ...
              switch (queueOperation) {
                ...
                case getLock:
                  redisAPI.hget(
                      locksKey,
                      body.getJsonObject(PAYLOAD).getString(QUEUENAME),
    ...
    uidMessageConsumer =
        eb.consumer(
            ...
            event -> {
              ...
              if (queue == null) {
                log.warn(
                    "Got event bus msg with empty body! uid={}  address={}  replyAddress={}",
                    uid,
                    event.address(),
                    event.replyAddress());
                // IMO we should 'fail()' here. But we don't, to keep backward compatibility.
              }
              log.debug("RedisQueues got notification for queue '{}'", queue);
              consume(queue);
            });
    ...
    vertx.setPeriodic(
        ...
        event -> {
          ...
          myQueues.entrySet().stream()
              .filter(entry -> entry.getValue() == QueueState.CONSUMING)
              .forEach(
                  entry -> {
                    ...
                    if (log.isTraceEnabled()) {
                      log.trace("RedisQueues refresh queues get: " + consumerKey);
                    }
                    redisAPI.get(
                        consumerKey,
                        getConsumerEvent -> {
                          if (getConsumerEvent.failed()) {
                            log.warn(
                                "Failed to get queue consumer for queue '{}'. But we'll continue anyway :)",
                                queue,
                                getConsumerEvent.cause());
                            // We should return here. See:
                          ...
                          if (uid.equals(consumer)) {
                            log.debug(
                                "RedisQueues Periodic consumer refresh for active queue " + queue);
                            refreshRegistration(queue, null);
                            updateTimestamp(queue, null);
                          } else {
                            log.debug("RedisQueues Removing queue " + queue + " from the list");
  ...
  private void enqueue(Message<JsonObject> event) {
    ...
    redisAPI.rpush(
        Arrays.asList(keyEnqueue, valueEnqueue),
        event2 -> {
          JsonObject reply = new JsonObject();
          if (event2.succeeded()) {
            if (log.isDebugEnabled()) {
              log.debug("RedisQueues Enqueued message into queue " + queueName);
            }
            ...
            String message =
                "RedisQueues QUEUE_ERROR: Error while enqueueing message into queue " + queueName;
  ...
  private void lockedEnqueue(Message<JsonObject> event) {
    log.debug("RedisQueues about to lockedEnqueue");
    JsonObject lockInfo =
        extractLockInfo(event.body().getJsonObject(PAYLOAD).getString(REQUESTED_BY));
    if (lockInfo != null) {
      redisAPI.hmset(
          Arrays.asList(
              locksKey,
              event.body().getJsonObject(PAYLOAD).getString(QUEUENAME),
              lockInfo.encode()),
          putLockResult -> {
            if (putLockResult.succeeded()) {
              log.debug("RedisQueues lockedEnqueue locking successful, now going to enqueue");
              enqueue(event);
            } else {
              log.warn("RedisQueues lockedEnqueue locking failed. Skip enqueue");
              event.reply(createErrorReply());
            }
          });
    } else {
      log.warn(
          "RedisQueues lockedEnqueue failed because property '" + REQUESTED_BY + "' was missing");
  ...
  private void getQueueItems(Message<JsonObject> event) {
    ...
    int maxQueueItemCountIndex =
        getMaxQueueItemCountIndex(event.body().getJsonObject(PAYLOAD).getString(LIMIT));
    redisAPI.llen(
        keyListRange,
        countReply -> {
          ...
          if (countReply.succeeded() && queueItemCount != null) {
            redisAPI.lrange(
                keyListRange,
                "0",
                String.valueOf(maxQueueItemCountIndex),
                new GetQueueItemsHandler(event, queueItemCount));
          } else {
            log.warn(
                "Operation getQueueItems failed. But I'll not notify my caller :)",
                countReply.cause());
            // IMO we should 'event.fail(countReply.cause())' here. But we don't, to keep backward
            // compatibility.
          }
        });
  }
  ...
  private void getQueues(
    ...
    if (filterPatternResult.isErr()) {
      event.reply(
          createErrorReply().put(ERROR_TYPE, BAD_INPUT).put(MESSAGE, filterPatternResult.getErr()));
    } else {
  ...
  private void getQueuesCount(Message<JsonObject> event) {
    ...
    if (result.getOk().isPresent()) {
      ...
      redisAPI.zcount(
          queuesKey,
          String.valueOf(getMaxAgeTimestamp()),
          String.valueOf(Double.MAX_VALUE),
          new GetQueuesCountHandler(event));
    }
  }
  ...
  private void replaceQueueItem(Message<JsonObject> event) {
    ...
    redisAPI.lset(
        keyReplaceItem,
        String.valueOf(indexReplaceItem),
        bufferReplaceItem,
        new ReplaceQueueItemHandler(event));
  }
  ...
  private void deleteQueueItem(Message<JsonObject> event) {
    ...
    redisAPI.lset(
        keyLset,
        String.valueOf(indexLset),
        "TO_DELETE",
        event1 -> {
          if (event1.succeeded()) {
            String keyLrem =
                queuesPrefix + event.body().getJsonObject(PAYLOAD).getString(QUEUENAME);
            redisAPI.lrem(
                keyLrem,
                "0",
                "TO_DELETE",
                replyLrem -> {
                  if (replyLrem.failed()) {
                    log.warn(
                        "Redis 'lrem' command failed. But will continue anyway.",
                        replyLrem.cause());
                    // IMO we should 'fail()' here. But we don't, to keep backward compatibility.
                  }
  ...
  private void deleteAllQueueItems(Message<JsonObject> event) {
    ...
    redisAPI.del(
        Collections.singletonList(buildQueueKey(queue)),
        deleteReply -> {
          if (deleteReply.failed()) {
            log.warn(
                "Failed to deleteAllQueueItems. But we'll continue anyway", deleteReply.cause());
          ...
          if (unlock) {
            redisAPI.hdel(
                Arrays.asList(locksKey, queue),
                unlockReply -> {
                  if (unlockReply.failed()) {
                    log.warn(
                        "Failed to unlock queue '{}'. Will continue anyway",
                        queue,
                        unlockReply.cause());
                    // IMO we should 'fail()' here. But we don't, to keep backward compatibility.
                  }
    ...
    if (sendSuccess) {
      ...
      if (queueConfiguration != null) {
        ...
        if (retryIntervals != null && retryIntervals.length > 0) {
          int retryIntervalIndex =
              (int)
                  (failureCount <= retryIntervals.length
                      ? failureCount - 1
  ...
  private void bulkDeleteQueues(Message<JsonObject> event) {
    ...
    if (!jsonArrayContainsStringsOnly(queues)) {
      event.reply(
          createErrorReply()
              .put(ERROR_TYPE, BAD_INPUT)
              .put(MESSAGE, "Queues must be string values"));
      return;
    }

    redisAPI.del(
        buildQueueKeys(queues),
        delManyReply -> {
  ...
  private void putLock(Message<JsonObject> event) {
    ...
    if (lockInfo != null) {
      JsonArray lockNames =
          new JsonArray().add(event.body().getJsonObject(PAYLOAD).getString(QUEUENAME));
      if (!jsonArrayContainsStringsOnly(lockNames)) {
        event.reply(
            createErrorReply()
                .put(ERROR_TYPE, BAD_INPUT)
                .put(MESSAGE, "Lock must be a string value"));
        return;
      }
  ...
  private void bulkPutLocks(Message<JsonObject> event) {
    ...
    if (!jsonArrayContainsStringsOnly(locks)) {
      event.reply(
          createErrorReply()
              .put(ERROR_TYPE, BAD_INPUT)
              .put(MESSAGE, "Locks must be string values"));
      return;
    }
  ...
  private void deleteLock(Message<JsonObject> event) {
    String queueName = event.body().getJsonObject(PAYLOAD).getString(QUEUENAME);
    redisAPI.exists(
        Collections.singletonList(queuesPrefix + queueName),
        event1 -> {
  ...
  private void getConfiguration(Message<JsonObject> event) {
    ...
    result.put(RedisQueuesConfiguration.PROP_ADDRESS, address);
    result.put(RedisQueuesConfiguration.PROP_REDIS_PREFIX, redisPrefix);
    result.put(RedisQueuesConfiguration.PROP_PROCESSOR_ADDRESS, processorAddress);
    result.put(RedisQueuesConfiguration.PROP_REFRESH_PERIOD, refreshPeriod);
    result.put(RedisQueuesConfiguration.PROP_REDIS_HOST, redisHost);
    result.put(RedisQueuesConfiguration.PROP_REDIS_PORT, redisPort);
    result.put(RedisQueuesConfiguration.PROP_REDIS_AUTH, redisAuth);
    result.put(RedisQueuesConfiguration.PROP_REDIS_ENCODING, redisEncoding);
    result.put(RedisQueuesConfiguration.PROP_CHECK_INTERVAL, checkInterval);
    result.put(RedisQueuesConfiguration.PROP_PROCESSOR_TIMEOUT, processorTimeout);
    result.put(RedisQueuesConfiguration.PROP_PROCESSOR_DELAY_MAX, processorDelayMax);
    result.put(
        RedisQueuesConfiguration.PROP_HTTP_REQUEST_HANDLER_ENABLED, httpRequestHandlerEnabled);
    result.put(RedisQueuesConfiguration.PROP_HTTP_REQUEST_HANDLER_PREFIX, httpRequestHandlerPrefix);
    result.put(RedisQueuesConfiguration.PROP_HTTP_REQUEST_HANDLER_PORT, httpRequestHandlerPort);
    result.put(
        RedisQueuesConfiguration.PROP_HTTP_REQUEST_HANDLER_USER_HEADER,
        httpRequestHandlerUserHeader);
  ...
  private void setConfiguration(Message<JsonObject> event) {
    ...
    setConfigurationValues(configurationValues, true)
        .onComplete(
            setConfigurationValuesEvent -> {
              if (setConfigurationValuesEvent.succeeded()) {
                log.debug(
                    "About to publish the configuration updates to event bus address '"
                        + configurationUpdatedAddress
                        + "'");
                ...
                event.reply(
                    createErrorReply()
                        .put(MESSAGE, setConfigurationValuesEvent.cause().getMessage()));
              }
            });
  }

  private Future<JsonObject> setConfigurationValues(
      JsonObject configurationValues, boolean validateOnly) {
    Promise<JsonObject> promise = Promise.promise();

    if (configurationValues != null) {
      List<String> notAllowedConfigurationValues =
          findNotAllowedConfigurationValues(configurationValues.fieldNames());
      if (notAllowedConfigurationValues.isEmpty()) {
        try {
          ...
          if (processorDelayMaxValue == null) {
            promise.fail(
                "Value for configuration property '" + PROCESSOR_DELAY_MAX + "' is missing");
            return promise.future();
          }
          if (!validateOnly) {
            this.processorDelayMax = processorDelayMaxValue;
            log.info(
                "Updated configuration value of property '"
                    + PROCESSOR_DELAY_MAX
                    + "' to "
                    + processorDelayMaxValue);
          }
          promise.complete(createOkReply());
        } catch (ClassCastException ex) {
          promise.fail(
              "Value for configuration property '" + PROCESSOR_DELAY_MAX + "' is not a number");
        }
      } else {
        String notAllowedConfigurationValuesString = notAllowedConfigurationValues.toString();
        promise.fail(
            "Not supported configuration values received: " + notAllowedConfigurationValuesString);
      }
    } else {
  ...
  private void registerQueueCheck(RedisQueuesConfiguration modConfig) {
    vertx.setPeriodic(
        modConfig.getCheckIntervalTimerMs(),
        periodicEvent ->
            luaScriptManager.handleQueueCheck(
                queueCheckLastexecKey,
                checkInterval,
                shouldCheck -> {
                  if (shouldCheck) {
  ...
  private void gracefulStop(final Handler<Void> doneHandler) {
    consumersMessageConsumer.unregister(
        event ->
            uidMessageConsumer.unregister(
                event1 -> {
  ...
  private void unregisterConsumers(boolean force) {
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues unregister consumers force: " + force);
    }
    log.debug("RedisQueues Unregistering consumers");
    for (final Map.Entry<String, QueueState> entry : myQueues.entrySet()) {
      final String queue = entry.getKey();
      if (force || entry.getValue() == QueueState.READY) {
        if (log.isTraceEnabled()) {
          log.trace("RedisQueues unregister consumers queue: " + queue);
        }
        refreshRegistration(
            queue,
            event -> {
              ...
              String consumerKey = consumersPrefix + queue;
              if (log.isTraceEnabled()) {
                log.trace("RedisQueues unregister consumers get: " + consumerKey);
              }
              redisAPI.get(
                  consumerKey,
                  event1 -> {
                    ...
                    if (log.isTraceEnabled()) {
                      log.trace("RedisQueues unregister consumers get result: " + consumer);
                    }
                    if (uid.equals(consumer)) {
                      log.debug("RedisQueues remove consumer: " + uid);
                      myQueues.remove(queue);
                    }
                  });
            });
  ...
  private void resetConsumers() {
    log.debug("RedisQueues Resetting consumers");
    String keysPattern = consumersPrefix + "*";
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues reset consumers keys: " + keysPattern);
    }
    redisAPI.keys(
        keysPattern,
        keysResult -> {
  ...
  private void consume(final String queueName) {
    if (log.isDebugEnabled()) {
      log.debug("RedisQueues Requested to consume queue " + queueName);
    }
    refreshRegistration(
        queueName,
        event -> {
          ...
          if (log.isTraceEnabled()) {
            log.trace("RedisQueues consume get: " + consumerKey);
          }
          redisAPI.get(
              consumerKey,
              event1 -> {
                ...
                String consumer = Objects.toString(event1.result(), "");
                if (log.isTraceEnabled()) {
                  log.trace("RedisQueues refresh registration consumer: " + consumer);
                }
                if (uid.equals(consumer)) {
                  QueueState state = myQueues.get(queueName);
                  if (log.isTraceEnabled()) {
                    log.trace(
                        "RedisQueues consumer: "
                            + consumer
                            + " queue: "
                            + queueName
                            + " state: "
                  ...
                  if (state != QueueState.CONSUMING) {
                    ...
                    if (state == null) {
                      ...
                      log.warn(
                          "Received request to consume from a queue I did not know about: "
                              + queueName);
                    }
                    if (log.isDebugEnabled()) {
                      log.debug("RedisQueues Starting to consume queue " + queueName);
                    }
                    readQueue(queueName);
                  } else {
                    if (log.isDebugEnabled()) {
                      log.debug("RedisQueues Queue " + queueName + " is already being consumed");
                    }
                  }
                } else {
  ...
  private void readQueue(final String queueName) {
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues read queue: " + queueName);
    }
    String queueKey = queuesPrefix + queueName;
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues read queue lindex: " + queueKey);
    }

    isQueueLocked(queueName)
        .onComplete(
            lockAnswer -> {
              ...
              if (!locked) {
                redisAPI.lindex(
                    ...
                    answer -> {
                      ...
                      if (log.isTraceEnabled()) {
                        log.trace("RedisQueues read queue lindex result: " + answer.result());
                      }
                      if (answer.result() != null) {
                        processMessageWithTimeout(
                            ...
                            success -> {
                              ...
                              int retryInterval =
                                  updateQueueFailureCountAndGetRetryInterval(queueName, success);

                              if (success) {
                                // Remove the processed message from the queue
                                if (log.isTraceEnabled()) {
                                  log.trace("RedisQueues read queue lpop: " + queueKey);
                                }
                                redisAPI.lpop(
                                    ...
                                    jsonAnswer -> {
                                      ...
                                      log.debug(
                                          "RedisQueues Message removed, queue "
                                              + queueName
                                              + " is ready again");
                                      myQueues.put(queueName, QueueState.READY);
                                      // Notify that we are stopped in case it was the last active
                                      // consumer
                                      ...
                                      if (log.isTraceEnabled()) {
                                        log.trace("RedisQueues read queue: " + queueKey);
                                      }
                                      redisAPI.llen(
                                          queueKey,
                                          answer1 -> {
                                            if (answer1.succeeded()
                                                && answer1.result() != null
                                                && answer1.result().toInteger() > 0) {
                                              notifyConsumer(queueName);
                                            }
                                          });
                                    });
                                ...
                                if (log.isDebugEnabled()) {
                                  log.debug("RedisQueues Processing failed for queue " + queueName);
                                  // reschedule
                                  log.debug(
                                      "RedisQueues will re-send the message to queue '"
                                          + queueName
                                          + "' in "
                                          + retryInterval
                                          + " seconds");
                        ...
                        // This can happen when requests to consume happen at the same moment the
                        // queue is emptied.
                        if (log.isDebugEnabled()) {
  ...
  private void rescheduleSendMessageAfterFailure(final String queueName, int retryInSeconds) {
    ...
    vertx.setTimer(
        ...
        timerId -> {
          if (log.isDebugEnabled()) {
            log.debug(
                "RedisQueues re-notify the consumer of queue '"
                    + queueName
                    + "' at "
          ...
          // reset the queue state to be consumed by {@link RedisQueues#consume(String)}
          myQueues.put(queueName, QueueState.READY);
        });
  }

  private void processMessageWithTimeout(
      final String queue, final String payload, final Handler<Boolean> handler) {
    if (processorDelayMax > 0) {
      log.info(
          "About to process message for queue "
              + queue
    ...
    timer
        .executeDelayedMax(processorDelayMax)
        .onComplete(
            delayed -> {
              if (delayed.failed()) {
                  ...
                  (Handler<AsyncResult<Message<JsonObject>>>)
                      reply -> {
                        ...
                        if (reply.succeeded()) {
                          ...
                          log.info(
                              "RedisQueues QUEUE_ERROR: Consumer failed "
                                  + uid
                                  + " queue: "
                                  + queue
                                  + " ("
  ...
  private void notifyConsumer(final String queueName) {
    log.debug("RedisQueues Notifying consumer of queue " + queueName);
    final EventBus eb = vertx.eventBus();

    // Find the consumer to notify
    String key = consumersPrefix + queueName;
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues notify consumer get: " + key);
    }
    redisAPI.get(
        key,
        event -> {
          ...
          if (log.isTraceEnabled()) {
            log.trace("RedisQueues got consumer: " + consumer);
          }
          if (consumer == null) {
            // No consumer for this queue, let's make a peer become consumer
            if (log.isDebugEnabled()) {
              log.debug("RedisQueues Sending registration request for queue " + queueName);
            }
            eb.send(address + "-consumers", queueName);
          } else {
            // Notify the registered consumer
            log.debug(
                "RedisQueues Notifying consumer " + consumer + " to consume queue " + queueName);
  ...
  private void refreshRegistration(String queueName, Handler<AsyncResult<Response>> handler) {
    if (log.isDebugEnabled()) {
      log.debug(
          "RedisQueues Refreshing registration of queue "
              + queueName
              + ", expire in "
              + consumerLockTime
              + " s");
  ...
  private void updateTimestamp(final String queueName, Handler<AsyncResult<Response>> handler) {
    ...
    if (log.isTraceEnabled()) {
      log.trace("RedisQueues update timestamp for queue: " + queueName + " to: " + ts);
    }
    if (handler == null) {
  ...
  /**
   * Notify not-active/not-empty queues to be processed (e.g. after a reboot). Check timestamps of
   * not-active/empty queues. This uses a sorted set of queue names scored by last update timestamp.
   */
  private void checkQueues() {
    ...
    redisAPI.zrangebyscore(
        Arrays.asList(queuesKey, "-inf", String.valueOf(limit)),
        answer -> {
          Response queues = answer.result();
          if (answer.failed() || queues == null) {
            log.error("RedisQueues is unable to get list of queues", answer.cause());
            return;
          }
          final AtomicInteger counter = new AtomicInteger(queues.size());
          if (log.isTraceEnabled()) {
            log.trace("RedisQueues update queues: " + counter);
          }
          for (Response queueObject : queues) {
            ...
            if (log.isTraceEnabled()) {
              log.trace("RedisQueues update queue: " + key);
            }
            redisAPI.exists(
                ...
                event -> {
                  if (event.failed() || event.result() == null) {
                    log.error(
                        "RedisQueues is unable to check existence of queue " + queueName,
                        event.cause());
                    return;
                  }
                  if (event.result().toLong() == 1) {
                    ...
                    updateTimestamp(
                        ...
                        result -> {
                          if (result.failed()) {
                            log.warn(
                                "Failed to update timestamps for queue '{}'",
                                queueName,
                                result.cause());
                            // We should return here. See:
                            // "https://softwareengineering.stackexchange.com/a/190535"
                    ...
                    // Make sure its TTL is correctly set (replaces the previous orphan detection
                    // mechanism).
                    ...
                    if (log.isTraceEnabled()) {
                      log.trace("RedisQueues remove old queue: " + queueName);
                    }
                    if (counter.decrementAndGet() == 0) {
                      removeOldQueues(limit);
                    }
      ...
      try {
        ...
        log.warn(
            "Invalid limit parameter '"
                + limit
                + "' configured for max queue item count. Using default "
                + DEFAULT_MAX_QUEUEITEM_COUNT);
      }
    }
   ...
   * @return null when no queueConfiguration's RegEx matches given queueName - else the
   *     QueueConfiguration
   */
  ...
  private void getQueuesItemsCount(
      Message<JsonObject> event, Result<Optional<Pattern>, String> filterPatternResult) {
    if (filterPatternResult.isErr()) {
      event.reply(
          createErrorReply().put(ERROR_TYPE, BAD_INPUT).put(MESSAGE, filterPatternResult.getErr()));
    } else {
      redisAPI.zrangebyscore(
          List.of(queuesKey, String.valueOf(getMaxAgeTimestamp()), "+inf"),
          new GetQueuesItemsCountHandler(
              event, filterPatternResult.getOk(), luaScriptManager, queuesPrefix));
    }
  }
  ...
  /**
   * Retrieve the queue statistics info of the requested queues filtered by the given filter
   * pattern.
   */
  private void getQueuesStatistics(
      Message<JsonObject> event, Result<Optional<Pattern>, String> filterPattern) {
    if (filterPattern.isErr()) {
      event.reply(
          createErrorReply().put(ERROR_TYPE, BAD_INPUT).put(MESSAGE, filterPattern.getErr()));
    } else {
      // retrieve all currently known queues from storage and pass this to the handler
      redisAPI.zrangebyscore(
          List.of(queuesKey, String.valueOf(getMaxAgeTimestamp()), "+inf"),
          new GetQueuesStatisticsHandler(event, filterPattern.getOk(), queueStatisticsCollector));
    }
  }
  ...
  /**
   * Retrieve the summarized queue speed of the requested queues filtered by the given filter
   * pattern.
   */
  private void getQueuesSpeed(
      Message<JsonObject> event, Result<Optional<Pattern>, String> filterPattern) {
    if (filterPattern.isErr()) {
      event.reply(
          createErrorReply().put(ERROR_TYPE, BAD_INPUT).put(MESSAGE, filterPattern.getErr()));
      ...
      redisAPI.zrangebyscore(
          List.of(queuesKey, String.valueOf(getMaxAgeTimestamp()), "+inf"),
          new GetQueuesSpeedHandler(event, filterPattern.getOk(), queueStatisticsCollector));
    }
  }
}
