package com.cheetahdigital.senderspoc.service.redisqueue.util;
...
import static com.cheetahdigital.senderspoc.service.redisqueue.util.RedisQueueAPI.PAYLOAD;
...
public class HttpServerRequestUtil {
  ...
  /**
   * Evaluates whether the provided request contains the provided url parameter and the value is
   * either an empty string or <code>true</code> (case ignored)
   *
   * @param parameter the url parameter to evaluate
   * @param request the http server request
   * @return returns true when request contains url parameter with value equal to <code>true</code>
   *     or empty string.
   */
  public static boolean evaluateUrlParameterToBeEmptyOrTrue(
      String parameter, HttpServerRequest request) {
  ...
  /**
   * Extracts a {@link JsonArray} from the provided request body. The {@link JsonArray} is not
   * allowed to be empty.
   *
   ...
   */
  public static Result<JsonArray, String> extractNonEmptyJsonArrayFromBody(
      String property, String requestBody) {
    try {
   ...
   * @return String
   */
  public static String encodePayload(String decoded) {
  ...
  public static String decode(String encoded) {
    ...
    for (Object headerObj : headers) {
      ...
      if (key.equalsIgnoreCase(CONTENT_TYPE)
          && (value.contains("text/") || value.contains(APPLICATION_JSON))) {
        try {
          object.put(
              PAYLOAD_OBJECT,
              new JsonObject(new String(object.getBinary(PAYLOAD), StandardCharsets.UTF_8)));
